...
function print_logs(){
	while read -r event; do
		...
		then
			SEEN[$eventId]=
			OUTPUT_LINE=""
			if [[ ! -v DISABLE_PRINT_GROUP ]]; then  OUTPUT_LINE+="$GROUP_COLOR$LOG_GROUP$NC " ; fi
			if [[ ! -v DISABLE_PRINT_STREAM ]]; then OUTPUT_LINE+="$STREAM_COLOR$logStreamName$NC " ; fi
			if [[ -v PRINT_TIMESTAMP ]]; then OUTPUT_LINE+="$TIMESTAMP_COLOR$(formatted_date $timestamp)$NC " ; fi
			if [[ -v PRINT_INGESTION ]]; then OUTPUT_LINE+="$INGESTION_COLOR$(formatted_date $ingestiontime)$NC " ; fi
			OUTPUT_LINE+=" $message"
			echo -e $OUTPUT_LINE
		fi
	done < <(aws logs filter-log-events --log-group-name $LOG_GROUP $AWS_LOGS_START_TIME $AWS_LOGS_END_TIME --query events[].[message,logStreamName,timestamp,ingestionTime,eventId] --output text $AWS_OPTIONS)
