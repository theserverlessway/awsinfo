...
FILTER=$(auto_filter jobQueueName status "join('',computeEnvironmentOrder[].computeEnvironment)" -- $FIRST_RESOURCE)
...
awscli batch list-jobs --job-queue $SELECTED --output table --job-status "$JOBS_STATUS" \
  --query "jobSummaryList[$(auto_filter jobName jobId  status statusReason -- $SECOND_RESOURCE)].{ \
    \"1.Id\":jobId, \
    \"2.Name\":jobName, \
    \"3.Status\":status, \
