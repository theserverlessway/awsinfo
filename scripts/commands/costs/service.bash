...
SERVICES=$(awscli ce get-cost-and-usage --time-period Start=$START_DATE,End=$END_DATE --granularity DAILY --metrics UNBLENDED_COST --group-by Type=DIMENSION,Key=SERVICE --output text --query "ResultsByTime[].Groups[$FILTER].[join('',Keys)]" | uniq)
